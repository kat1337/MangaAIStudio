...
stopped_at: Completed fast follow-up 260909-hang — SIGABRT post-mortem from first real mas-hang.log; native-crash minidumps + heartbeat memory telemetry added
last_updated: "2026-09-09T23:59:00.000Z"
last_activity: 2026-09-09
last_activity_desc: Completed fast follow-up 260909-hang — mas-hang.log SIGABRT post-mortem (native abort under app.exec(), zero Python frames, no Python-side defect findable) → diagnostics hardened with Windows crash minidumps (mas-*.dmp via MiniDumpWriteDump, faulthandler-chaining SIGABRT path) + psutil memory telemetry in the heartbeat (latched warnings at 8GB RSS / 90% commit); suite 1458 passed + 2 baseline flakes
progress:
  total_phases: 10
  completed_phases: 10
  total_plans: 76
...
Plan: —
Status: Awaiting next milestone
Last activity: 2026-09-09 - Completed fast follow-up 260909-hang: SIGABRT post-mortem from the first real mas-hang.log — dump proved native abort() under app.exec() with zero Python frames (excepthook + qFatal routing both silent → uncaught C++ exception class, bad_alloc prime suspect given torch/scipy residency + 35MP pages); code hunt found no demonstrable Python-side defect (graveyard used at all 3 last-ref sites, ke1 editor guard present, copy path text-only). Diagnostics hardened: Windows crash minidumps (SetUnhandledExceptionFilter → MiniDumpWriteDump DataSegs ~1.8MB → mas-*.dmp next to mas.log; ucrtbase _set_abort_behavior + C-level ctypes SIGABRT handler that chains faulthandler so the all-thread dump survives; once-per-session latch, bounded logger.complete, fully guarded) with end-to-end subprocess-abort proof; heartbeat memory telemetry via psutil on a daemon thread (rss/commit INFO ~1/min, latched warn-once at 8192MB RSS / 90% commit, env-overridable MAS_MEM_*; psutil warmed pre-watchdog + declared in pyproject); suite 1458 passed (12 new diagnostics tests) + 2 baseline clipboard flakes; Needs Review (next native crash: read mas-*.dmp)
