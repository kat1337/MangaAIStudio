...
- **pyproject.toml pins** `numpy<2` but the working env has numpy 2.3.5 with the Phase 1–4 suite green at plan time (measured 2026-08-08: 462 collected / 461 passed / 1 failed — the sole failure is the pre-existing tests/test_gui_boxes.py::test_moved_box_via_real_events_persists_round_trip 1px move-round-trip regression, a test-side QTest int-truncation artifact; remediation in plan 05-09) — do not re-pin; do not add new dependencies. [VERIFIED: pyproject.toml:25, local probe]
...
**Packages flagged as suspicious [SUS]:** all five above carry the seam's `unknown-downloads` reason — a data-availability gap, not a real signal. No postinstall scripts (`postinstall: null` for all), no slop signals, all long-established projects, all already installed and exercised by the Phase 1–4 suite (462 collected / 461 passed / 1 failed at plan time — the single failure is a pre-existing test-side regression unrelated to packages, fixed by plan 05-09). **No `checkpoint:human-verify` tasks are needed because nothing is installed in this phase.**
...
|------------|------------|-----------|---------|----------|
| Python (pyenv) | Runtime | ✓ | 3.14.2 (pyenv-win default) | — (project already runs here; 462 collected / 461 passed / 1 failed at plan time — sole pre-existing failure fixed by plan 05-09) |
