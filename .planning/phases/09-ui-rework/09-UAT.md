...
number: 1
name: Backstop — strip accent-border visual check
expected: |
  With the app running at 100% zoom, select each tool on the strip — the checked button renders the 1px #00d4ff accent border on its :checked state; unchecked buttons sit flat on #2d2d33 with a 1px #3a3a42 border. Visible cyan accent border on the active tool button only; flat dark chrome on the rest.
