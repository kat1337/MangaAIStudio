...
### Vendoring (NEW — supersedes the prior "consume read-only, no vendoring" assumption)
- **D-14:** **Vendor PanelCleaner's `structures.py` AND `masker.py` into `panelcleaner/`**, near-verbatim per Phase 1 D-12 (GPL v3 → GPL v3, license-compatible). This supersedes the earlier assumption (recorded in the UI-SPEC registry row and the Claude's-Discretion "Box wrapper" item) that Phase 3 would only *consume* `TextBlock` read-only with no new vendoring.
  - **`pcleaner/structures.py` → `panelcleaner/structures.py`** — provides the canonical `Box` dataclass (`@frozen` x1/y1/x2/y2) + `BoxType` enum (`BOX` / `EXTENDED_BOX` / `MERGED_EXT_BOX` / `REFERENCE_BOX`) + an `as_tuple_xywh` property that maps directly to `QRectF(x, y, w, h)`. This **becomes the `Box` model for Phase 3** (resolving the Claude's-Discretion "Box wrapper" item in favor of vendoring PanelCleaner's rather than hand-rolling one) and gives every box a clean bbox type that already speaks Qt rect coordinates. The `origin` flag (D-03 "detected" vs "user") and stable identity for undo (D-10) layer ON TOP of this vendored `Box` (a thin subclass or a parallel tracks-origin struct) — do not mutate the vendored `Box` itself.
  - **`pcleaner/masker.py` → `panelcleaner/masker.py`** — PanelCleaner's mask-refinement + box/mask interaction logic. Phase 1 deferred this file; Phase 3 vendors it because D-15 (below) needs its per-box mask handling and std-deviation checks. Researcher confirms the exact surface Phase 3 consumes (box→mask relationship, std-deviation routines) and whether `masker.py` pulls in further deps (`output_structures.py`, `analytics.py`) that must come along transitively — flag any transitive-vendor chain in RESEARCH.md.
  - **D-12 discipline applies:** adapt near-verbatim; preserve GPL v3 headers; do NOT vendor MangaCleaner_GPU (still reference-only). The CTD subtree (`comic_text_detector/`, incl. `textblock.py` + `textmask.py`) was already vendored in Phase 1 and is NOT re-touched.
- **D-15:** **Plan-for (but defer the inpaint itself) a "selective per-box inpaint" seam driven by masker's std-deviation checks.** The goal (user's vision): instead of masking/inpainting an entire box region, inpaint ONLY the detected masks *inside* a box when the box's std deviation is low enough (i.e. the box region is visually uniform enough that inpainting the text masks clean is safe). PanelCleaner's `masker.py` std-deviation machinery is the basis. **Phase 3 does NOT implement the inpaint** (that is a later phase — it depends on Phase 4 OCR context and is out of TEXT-01/TEXT-03 scope); Phase 3's job is to (a) vendor `masker.py` so the machinery exists, and (b) structure the box/mask data model so a later phase can compute per-box std-dev and selectively inpaint without rework. Researcher identifies the exact `masker.py` functions involved and the data-model shape that leaves the seam open; planner records the deferred inpaint as a follow-up. **This is a plan-for-it decision, not a build-it-now decision** — surface it explicitly so the planner doesn't accidentally close the seam.
...
- `../PanelCleaner/LICENSE` — GPL v3; derivative works must be GPL v3.

### Vendoring Candidates (NEW — D-14, supersedes the prior "no vendoring" assumption)
**These two PanelCleaner files WILL be vendored into `panelcleaner/` near-verbatim per Phase 1 D-12 (GPL v3 → GPL v3). Researcher confirms the exact surface + any transitive-dep chain (e.g. does `masker.py` import `output_structures.py`/`analytics.py` that must come along?).**
- `../PanelCleaner/pcleaner/structures.py` → `panelcleaner/structures.py` — **the `Box` model for Phase 3.** `@frozen` dataclass `Box(x1,y1,x2,y2)` + `BoxType` enum (`BOX`/`EXTENDED_BOX`/`MERGED_EXT_BOX`/`REFERENCE_BOX`) + `as_tuple` / `as_tuple_xywh` properties (the latter maps directly to `QRectF(x, y, w, h)` — Qt-ready). `__contains__` for point-in-box hit-testing. ~70 lines, attrs-based, low dep surface (PIL, attrs, loguru — all already in the env). This RESOLVES the "Box wrapper" question: vendor PanelCleaner's `Box` rather than hand-rolling.
- `../PanelCleaner/pcleaner/masker.py` → `panelcleaner/masker.py` — mask-refinement + box/mask interaction logic. Phase 1 deferred this; Phase 3 vendors it because D-15 (selective per-box inpaint via std-deviation) needs its per-box mask handling. Researcher maps: which functions compute per-box masks, which compute std-deviation, and what `masker.py` imports transitively (`output_structures.py`? `analytics.py`? `helpers.py`?). Flag the full transitive-vendor set in RESEARCH.md so the planner vendors a self-consistent slice.
...
### Licensing
- `../PanelCleaner/LICENSE` — GPL v3. Phase 3 vendors `structures.py` + `masker.py` near-verbatim per Phase 1 D-12 (GPL v3 → GPL v3, license-compatible); the box-layer UI code (`BoxItem`, handles, layer) is our own; consumption of `TextBlock` from the Phase-1-vendored CTD tree is derivative use. MangaCleaner_GPU remains reference-only (no LICENSE).
...
- **Selective per-box inpaint via std-deviation (D-15)** — DEFERRED but PLAN-FOR-IT. Inpaint ONLY the detected masks *inside* a box when the box's std deviation is low enough (uniform-enough region), rather than masking/inpainting the whole box. Basis: the `masker.py` std-deviation machinery vendored per D-14. **Phase 3 vendors `masker.py` and keeps the data-model seam open; it does NOT implement the inpaint** (that depends on Phase 4 OCR context and is out of TEXT-01/TEXT-03 scope). Likely lands in a later phase (Phase 4 OCR track or a decimal insertion). Planner records this as an explicit follow-up and must NOT close the seam.
