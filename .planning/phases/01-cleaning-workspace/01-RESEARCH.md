...
- **Adapters + adapted source layout** — `adapters/` (model interfaces, our own), `panelcleaner/` (PanelCleaner code vendored near-verbatim per D-12: config/detection/inpainting/viewer), `gui/`+`core/` (our own, incl. the interactive mask-editing canvas for CLEAN-03/04/05 — patterned after MangaCleaner_GPU but NOT vendored, per D-12), `config/` (PanelCleaner config)
- **Adaptation/licensing policy (D-12)** — PanelCleaner: verbatim (GPL v3, compatible). MangaCleaner_GPU (`~/Downloads/MangaCleaner_GPU/_internal/src/`): **reference-only reimplementation** — ships as a no-LICENSE binary distribution, so its code is all-rights-reserved and must not be copied; read for patterns, reimplement in our own code.
