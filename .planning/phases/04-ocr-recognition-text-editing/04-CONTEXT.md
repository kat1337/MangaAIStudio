...
- **Typesetting styling controls (font/style/size/color/alignment/effects) are DEFERRED, not forgotten.** The user raised mid-discussion that a manga editor's text editor needs font selection (per-box / per-selected-boxes / per-page), font style, font size +/-, font color, H/V alignment, and effects (outer glow, outline, etc.). This is TRAN-02 (typesetting) territory — explicitly Out-of-Scope in PROJECT.md and reinforced by D-09/D-11 (Phase 4's overlay is review/correction, not output). **The user chose to defer the full styling toolbar to a dedicated typesetting phase (TRAN-02 / Phase 5.5 insertion) rather than expand Phase 4.** Phase 4's review overlay uses hardcoded sensible defaults only. The full feature list is preserved in the Deferred Ideas section for the typesetting phase to inherit. (Decision recorded 2026-08-04.)
...
- **Typesetting / rendering translated text into the page (TRAN-02)** — v2; PROJECT.md Out of Scope. Phase 4 renders text as a translucent review/correction overlay (D-11), not as final typeset output (no font management, auto-fit, bubble-sizing — that's BallonsTranslator territory). **The full typesetting feature list deferred with this decision (2026-08-04, raised mid-discussion and explicitly deferred by the user):** font selection (per-box, per-selected-boxes, per-page), font style selector, font size selector, increase/decrease font size, font color, horizontal alignment, vertical alignment, effects (outer glow, outline, etc.). "Per selected boxes" additionally implies multi-select (currently single-select per Phase 3 D-08). A future typesetting phase (TRAN-02, or a Phase 5.5 insertion) inherits this list. Phase 4's review overlay uses hardcoded/sensible-default presentation only (D-11).
