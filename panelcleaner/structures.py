...
"""
Data structures for the batch masker / OCR / inpaint pipeline (PageData,
MaskData, MaskerData, MaskFittingResults, InpainterData, etc.) plus the
``Box`` frozen bbox and ``BoxType`` enum.

This replaces the 18-line Phase 1 placeholder stub. The full surface is the
load-bearing requirement: ``panelcleaner.image_ops.pick_best_mask`` constructs
``st.MaskFittingResults(...)`` at image_ops.py:706,717 and the stub defined
nothing — Phase 3 (plan 03-01) vendors the real module per D-14 so the std-dev
seam (D-15) is genuinely callable.
"""
...
def merge_ocr_analytics(
    ...
    # box_sizes_ocr: Sequence[int]
    # box_sizes_removed: Sequence[int]
    # removed_box_data: Sequence[Path, str, Box]
