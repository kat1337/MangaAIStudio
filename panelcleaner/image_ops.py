...
# Vendored near-verbatim from PanelCleaner pcleaner/image_ops.py (GPL v3) per
# CONTEXT.md D-12. Imports rewritten pcleaner. -> panelcleaner. The
# ``from __future__ import annotations`` added below makes the st.Box /
# st.MaskFittingResults / cfg.MaskerConfig annotations lazy (never resolved at
# def-time), so the module imports cleanly even though the batch-pipeline
# structures module (panelcleaner/structures.py) is a Phase 2 placeholder.
"""
Image operations (crop, rotate, levels, mask<->RGBA conversions). Centralized
mask-format path (RESEARCH Pitfall 6). Phase 1 interactive inpainting consumes
only the mask<->RGBA conversion helpers; the batch masker/denoiser/inpaint
helpers are Phase 2 scope (FLOW-03) and kept verbatim for parity.
"""
...
def visualize_standard_deviations(
    ...
    font_path = hp.resource_path(panelcleaner.data, "LiberationSans-Regular.ttf")
