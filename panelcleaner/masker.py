...
# Vendored near-verbatim from PanelCleaner pcleaner/masker.py (GPL v3) per
# CONTEXT.md D-12/D-14. Imports rewritten pcleaner. -> panelcleaner.
"""
Batch mask-fitting driver (PanelCleaner ``pcleaner/masker.py``).

``mask_page`` is the batch-GUI analytics driver that takes a
``MaskerData`` (json path + cache dir + masker config) and produces per-box
mask-fitment analytics via ``image_ops.pick_best_mask``. It is DEAD CODE in
Manga AI Studio's context (Phase 3 never calls it), but is vendored
near-verbatim for two reasons (RESEARCH Open Question 3):
  1. Upstream diffability — a near-verbatim file is easier to diff against
     the PanelCleaner source than a carved subset.
  2. The std-dev seam (D-15) — a later per-box-inpaint phase calls
     ``image_ops.pick_best_mask`` / ``image_ops.border_std_deviation`` directly;
     having the canonical driver present documents the call sequence.

The one deviation from upstream is the ``output_structures`` import guard
below (RESEARCH Pitfall 1): ``panelcleaner.output_structures`` (the batch GUI
analytics pipeline) is deliberately NOT vendored, so the bare import is
wrapped in try/except ImportError. ``ost = None`` is harmless because
``mask_page`` (the only consumer) is never called in our context.
"""
